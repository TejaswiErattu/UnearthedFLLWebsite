...
async function fetchPageText(url) {
  try {
    ...
    // Simple HTML parsing without cheerio for Vercel edge compatibility
...
async function webSearch(q) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const out = { results: [], error: `CSE error ${response.status}: ${txt.slice(0, 200)}` };
      cache.set(key, out);
      return out;
    }
    const data = await response.json();
    ...
    const out = { results: [], error: `Search failed: ${error.message}` };
    cache.set(key, out);
    return out;
  }
}
